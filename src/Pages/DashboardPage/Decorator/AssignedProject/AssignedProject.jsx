...
const AssignedProject = () => {
    ...
    const handleStatusChange = async(bookingId, status)=>{
        try {
            await axiosSecure.patch(`/decorator/project/${bookingId}/status`, {status});
    ...
    return (
        <div className='p-6'>
            ...
            {
                projects.length === 0 ? (
                    <p className='text-center text-accent py-10'>No assigned projects found.</p>
                ) : (
                            ...
                            <table className="table">
                              ...
                              <thead className="bg-base-200 ">
                                <tr>
                                  ...
                                  <th className="text-primary font-bold text-lg">Update Status</th>
                                </tr>
                              </thead>
                              <tbody>
                                  ...
                                  <tr key={booking._id}>
                                    ...
                                    <td className="p-2">
                                     ...
                                     className={`badge text-white ${
                                        ...
                                        booking.status === 'on_the_way' ? 'bg-blue-500' :
                                        booking.status === 'materials_ready' ? 'bg-yellow-500' :
                                        booking.status === 'planning' ? 'bg-purple-500' : 
                                        'bg-gray-500'
                                     }`}
                                     >
                                    ...
                                    <td className='p-2'>
                                        <select value={booking.status || 'assigned'} onChange={(e)=> handleStatusChange(booking._id, e.target.value)}
