...
const AssignDecorators = () => {
   ...
   const {data: bookings= [], isLoading: bookingLoading, refetch } = useQuery({
        ...
        >
          ...
          {decorators.map(d => (
            <option key={d._id} value={d._id}>
              { d.displayName} (
            {Array.isArray(d.specialties) && d.specialties.length > 0 ? d.specialties.join(', ') : 'No specialties'}
            )
