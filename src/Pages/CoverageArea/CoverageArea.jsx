...
const CoverageArea = () => {
    ...
    // console.log(servicesArea)
        const position = [23.6850, 90.3563];
           ...
           const matchedService = servicesArea.find(service=> service.locations.some(loc=>
            loc.city.toLowerCase().includes(location)
           ))
           if(!matchedService) return;
           const findLocation = matchedService.locations.find(loc=> loc.city.toLowerCase().includes(location))
    ...
    return (
        ...
        <div
  className="relative w-[350px] md:w-[400px] lg:w-[480px] bg-gray-100 rounded-2xl shadow-md p-1 hover:shadow-lg"
>
  <div
  ...
  />
  <button
    className="absolute right-1 top-1 bottom-1 px-6 bg-primary text-white font-medium rounded-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#2E7D32]"
  >
    Search
  </button>
</div>
