...
const EditServiceModal = ({isEditOpen, setIsEditOpen, refetch, editService}) => {
 const axiosSecure = useAxiosSecure();
    ...
    useEffect(()=>{
        if(editService){
            reset({
                ...
                service_category: editService.service_category,
    ...
    const handleEditService = async(data)=>{
     ...
     try {
        ...
        const res = await axiosSecure.patch(`/services/${editService._id}`, serviceData);
        if(res.data.modifiedCount){
            toast.success('Update service info successfully!')
            refetch();
            closeModal();
        }
                   ...
                   leaveTo='opacity-0 scale-95'
                           ...
                           >
                               <DialogPanel className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
                                   ...
                                   <form onSubmit={handleSubmit(handleEditService)} className='space-y-4'>
                                       <div>
                                       ...
                                       <div className='flex justify-end gap-2 pt-4'>
                                           <button type='submit' className='btn bg-primary text-white hover:bg-secondary'>Update Service</button>
